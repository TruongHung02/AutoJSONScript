...
export default async function getOTP(username: string, password: string, proxy?: string) {
  ...
  // else
  if (mailServer === 'veer') return getOTPVeer(username, password, proxy)
...
async function getOTPVeer(username: string, password: string, proxy?: string) {
  ...
  try {
    ...
    logger.error(`Account: ${username} ${error}`)
  }
...
async function getOTPBiz(username: string, password: string) {
  ...
  if (!latestEmail) {
    throw new Error(`Account: ${username} Cant get mail`)
  }
  ...
  if (!codeIndex) {
    throw new Error(`Account: ${username} Cant get code`)
  }
