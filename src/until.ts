...
export function waitForXpathSelector(page: Page, xpathSelector: string, timeout: number = 30000) {
  const timeoutPromise = new Promise<ElementHandle<Element> | null>((_, reject) =>
    setTimeout(() => reject(new Error(`Cant find element with Xpath selector: ${xpathSelector}`)), timeout),
  )
